...
def _sqlite_journal_mode() -> str:
    ...
    mounts ``/home``. Using WAL there corrupts the database file under concurrent
    writes ("database disk image is malformed"). So on such paths we fall back to
    the classic rollback journal (DELETE), which works correctly over a network
    share. ``SQLITE_JOURNAL_MODE`` can force a specific mode if ever needed.
    """
